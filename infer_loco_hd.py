...
def infer(args):
    ...
    if args.students_blocks == 'Both_ViT':
        # --- ViT Bidirectional ---
        teachers['fe'] = HDViTFeatureExtractor(layers=[1, 5]).to(device, dtype=dtype).eval()
        
        students['backward_net'] = ResidualFeatureProjectionMLP(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, reduction_factor=1).to(device, dtype=dtype)
        students['forward_net'] = ResidualFeatureProjectionMLP(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, reduction_factor=1).to(device, dtype=dtype)
        ...
        teachers['fe'] = HDLLMFeatureExtractor(
            conversation_template=PROMPTS_CONFIG[f'distilled_{args.class_name}'], layers=[0, 1]
        ).to(device).eval()
        
        students['backward_net'] = FeatureProjectionMLP(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim,
                                                        reduction_factor=0.4).to(device=device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim,
                                                       reduction_factor=0.4).to(device=device, dtype=dtype)
        ...
        teachers['llm_fe'] = HDLLMFeatureExtractor(
            conversation_template=PROMPTS_CONFIG['generic'], layers=[0, 1]
        ).to(device).eval()
    ...
    for batch_data in tqdm(test_loader, desc=f'Inferencing {args.class_name}'):
        ...
        with torch.no_grad():
            ...
            if args.students_blocks == 'Both_ViT':
                ...
                B, H, W, C = first_patch.shape
                first_flat = first_patch.view(-1, C)
                second_flat = second_patch.view(-1, C)
                # Predictions
                pred_1st_flat = students['backward_net'](second_flat)
                pred_2nd_flat = students['forward_net'](first_flat)
                pred_1st_patch = pred_1st_flat.view(B, H, W, C)
                pred_2nd_patch = pred_2nd_flat.view(B, H, W, C)
                # Map Calc
                ...
                # Extraction
                vit_1st, vit_2nd = teachers['vit_fe'](input_data)
                llm_1st, llm_2nd = teachers['llm_fe'](input_data)
                
                # ViT: Flatten and Predict
                B_vit, H_vit, W_vit, C_vit = vit_1st.shape
                vit_1st_flat = vit_1st.view(-1, C_vit)
                vit_2nd_flat = vit_2nd.view(-1, C_vit)
                # Predict
                pred_vit_2nd_flat = students['vit_fw'](vit_1st_flat)
                pred_vit_1st_flat = students['vit_bw'](vit_2nd_flat)
                # Unflatten back
                pred_vit_2nd = pred_vit_2nd_flat.view(B_vit, H_vit, W_vit, C_vit)
                pred_vit_1st = pred_vit_1st_flat.view(B_vit, H_vit, W_vit, C_vit)

                # LLM: Flatten and Predict
                B_llm, H_llm, W_llm, C_llm = llm_1st.shape
                llm_1st_flat = llm_1st.view(-1, C_llm)
                llm_2nd_flat = llm_2nd.view(-1, C_llm)
                # Predict
                pred_llm_2nd_flat = students['llm_fw'](llm_1st_flat)
                pred_llm_1st_flat = students['llm_bw'](llm_2nd_flat)
                # Unflatten back
                pred_llm_2nd = pred_llm_2nd_flat.view(B_llm, H_llm, W_llm, C_llm)
                pred_llm_1st = pred_llm_1st_flat.view(B_llm, H_llm, W_llm, C_llm)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label', default='hd_generic_0.4', type=str,
                        help='Experiment label.')
    
    parser.add_argument('--vit_label', default='hd_norm_11_15', type=str,
