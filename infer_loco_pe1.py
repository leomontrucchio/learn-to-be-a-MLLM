...
from utils_loco.general_utils import set_seeds, extract_metrics, siglip_denormalize, SinusoidalPositionalEmbedding2D
from utils_loco.config_loco import PROMPTS_CONFIG

from models.teacher import ViTFeatureExtractor, LLMFeatureExtractor
from models.student import FeatureProjectionMLP_TrainableScaleSum, FeatureProjectionMLP, FeatureProjectionMLP_TrainableScale
...
def infer(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        students['backward_net'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, pe_dim=pe_dim, reduction_factor=0.4).to(device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, pe_dim=pe_dim, reduction_factor=0.4).to(device, dtype=dtype)
        ...
        teachers['fe'] = LLMFeatureExtractor(conversation_template=PROMPTS_CONFIG[f'generic'],
                                             layer1_idx=args.llm_layers[0], layer2_idx=args.llm_layers[1]).to(device).eval()
        
        pe_dim = teachers['fe'].embed_dim

        pe_generators['llm'] = SinusoidalPositionalEmbedding2D(pe_dim=pe_dim, size_h=14).to(device, dtype=dtype)
        
        students['backward_net'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, pe_dim=pe_dim, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device=device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, pe_dim=pe_dim, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device=device, dtype=dtype)
        ...
        pe_dim_vit = teachers['vit_fe'].embed_dim
        pe_dim_llm = teachers['llm_fe'].embed_dim
        pe_generators['vit'] = SinusoidalPositionalEmbedding2D(pe_dim=pe_dim_vit, size_h=grid_size_vit).to(device, dtype=dtype)
        pe_generators['llm'] = SinusoidalPositionalEmbedding2D(pe_dim=pe_dim_llm, size_h=14).to(device, dtype=dtype)

        # ViT Students
        students['vit_fw'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, pe_dim=pe_dim_vit, reduction_factor=0.4).to(device=device, dtype=dtype)
        students['vit_bw'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, pe_dim=pe_dim_vit, reduction_factor=0.4).to(device=device, dtype=dtype)
        
        # LLM Students
        students['llm_fw'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, pe_dim=pe_dim_llm, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device=device, dtype=dtype)
        students['llm_bw'] = FeatureProjectionMLP_TrainableScaleSum(in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, pe_dim=pe_dim_llm, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device=device, dtype=dtype)
    ...
    for pil_img, tensor_img, img_path_list in tqdm(test_loader, desc=f'Inferencing {args.class_name}'):
        ...
        # Calculate how the image was scaled to fit 384x384 based on ImageOps.pad
        ...
        with torch.no_grad():
            ...
            if args.students_blocks == 'Both_ViT':
                ...
                # Prediction
                pred_1st_patch = students['backward_net'](second_patch, pe)
                pred_2nd_patch = students['forward_net'](first_patch, pe)
                ...
                # PE
                pe_vit = pe_generators['vit'](vit_1st)
                pe_llm = pe_generators['llm'](llm_1st)

                # Prediction
                pred_vit_2nd = students['vit_fw'](vit_1st, pe_vit)
                pred_vit_1st = students['vit_bw'](vit_2nd, pe_vit)
                pred_llm_2nd = students['llm_fw'](llm_1st, pe_llm)
                pred_llm_1st = students['llm_bw'](llm_2nd, pe_llm)
            ...
            # Upsample to the Model's Input Size
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vit_label', default='ds_vit_pe010_11_15', type=str,
