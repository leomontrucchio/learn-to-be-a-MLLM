...
from models.student import ResidualFeatureProjectionMLP, FeatureProjectionMLP, FeatureProjectionMLP_FullyTrainable
...
def infer(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        students['backward_net'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size, act_layer=torch.nn.GELU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size, act_layer=torch.nn.GELU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        ...
        students['backward_net'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size, act_layer=torch.nn.SiLU, reduction_factor=0.4
        ...
        students['vit_fw'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size_vit, act_layer=torch.nn.GELU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['vit_bw'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=grid_size_vit, act_layer=torch.nn.GELU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        
        # LLM Students (Standard)
        students['llm_fw'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=14, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['llm_bw'] = FeatureProjectionMLP_FullyTrainable(
            in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, 
            pe_dim=pe_dim, grid_size=14, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
