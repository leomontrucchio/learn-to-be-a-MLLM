...
from models.student import FeatureProjectionMLP_FullyTrainableSum, FeatureProjectionMLP, FeatureProjectionMLP_FullyTrainable
...
def infer(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        students['backward_net'] = FeatureProjectionMLP_FullyTrainableSum(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            grid_size=grid_size, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_FullyTrainableSum(
            in_features=teachers['fe'].embed_dim, out_features=teachers['fe'].embed_dim, 
            grid_size=grid_size, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        ...
        students['vit_fw'] = FeatureProjectionMLP(in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, reduction_factor=0.4).to(device, dtype=dtype)
        students['vit_bw'] = FeatureProjectionMLP(in_features=teachers['vit_fe'].embed_dim, out_features=teachers['vit_fe'].embed_dim, reduction_factor=0.4).to(device, dtype=dtype)
        
        # LLM Students (Standard)
        students['llm_fw'] = FeatureProjectionMLP_FullyTrainableSum(
            in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, 
            grid_size=14, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
        students['llm_bw'] = FeatureProjectionMLP_FullyTrainableSum(
            in_features=teachers['llm_fe'].embed_dim, out_features=teachers['llm_fe'].embed_dim, 
            grid_size=14, act_layer=torch.nn.SiLU, reduction_factor=0.4
            ).to(device, dtype=dtype)
    ...
    for pil_img, tensor_img, img_path_list in tqdm(test_loader, desc=f'Inferencing {args.class_name}'):
        ...
        with torch.no_grad():
            ...
            if args.students_blocks == 'Both_ViT':
                ...
                combined_anomaly_map = torch.maximum(vit_comb, llm_comb)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label', default='ds_llm_pe_fulltrainsum_conc_0_1', type=str,
