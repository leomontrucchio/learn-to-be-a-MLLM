...
from models.student import FeatureProjectionMLP_FullyTrainableSumHD, QwenAlignedStudent, QwenViTAlignedStudent
...
def infer(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        students['fw'] = FeatureProjectionMLP_FullyTrainableSumHD(in_features=C, out_features=C, num_patches=num_patches, act_layer=torch.nn.GELU, reduction_factor=0.4).to(device, dtype)
        students['bw'] = FeatureProjectionMLP_FullyTrainableSumHD(in_features=C, out_features=C, num_patches=num_patches, act_layer=torch.nn.GELU, reduction_factor=0.4).to(device, dtype)

        students['fw'].load_state_dict(torch.load(os.path.join(check_dir, f'forward_net_{args.vit_label}_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'), map_location=device))
        students['bw'].load_state_dict(torch.load(os.path.join(check_dir, f'backward_net_{args.vit_label}_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'), map_location=device))
        ...
        conv_template = PROMPTS_QWEN['assistant_generic'] 
        with torch.no_grad():
            feat, _ = teachers['fe'](pil_imgs_sample, conv_template)
        B, H, W, C = feat.shape
        num_patches = H * W

        students['fw'] = FeatureProjectionMLP_FullyTrainableSumHD(in_features=C, out_features=C, num_patches=num_patches, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device, dtype)
        students['bw'] = FeatureProjectionMLP_FullyTrainableSumHD(in_features=C, out_features=C, num_patches=num_patches, act_layer=torch.nn.SiLU, reduction_factor=0.4).to(device, dtype)

        students['fw'].load_state_dict(torch.load(os.path.join(check_dir, f'forward_net_{args.label}_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'), map_location=device))
        students['bw'].load_state_dict(torch.load(os.path.join(check_dir, f'backward_net_{args.label}_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'), map_location=device))
        ...
        with torch.no_grad():
            fv, _ = teachers['vit_fe'](pil_imgs_sample)
            fl, _ = teachers['llm_fe'](pil_imgs_sample, conv_template)
        
        num_v, dim_v = fv.shape[1]*fv.shape[2], fv.shape[-1]
        num_l, dim_l = fl.shape[1]*fl.shape[2], fl.shape[-1]

        # Inizializzazione Studenti Full
        students['vit_fw'] = FeatureProjectionMLP_FullyTrainableSumHD(dim_v, dim_v, num_v, torch.nn.GELU, 0.4).to(device, dtype)
        students['vit_bw'] = FeatureProjectionMLP_FullyTrainableSumHD(dim_v, dim_v, num_v, torch.nn.GELU, 0.4).to(device, dtype)
        students['llm_fw'] = FeatureProjectionMLP_FullyTrainableSumHD(dim_l, dim_l, num_l, torch.nn.SiLU, 0.4).to(device, dtype)
        students['llm_bw'] = FeatureProjectionMLP_FullyTrainableSumHD(dim_l, dim_l, num_l, torch.nn.SiLU, 0.4).to(device, dtype)
    ...
    for pil_imgs, img_path_list in tqdm(test_loader, desc=f'Inference {args.class_name}'):
        ...
        with torch.no_grad():
            if args.students_blocks == 'Full':
                ...
                # ViT Anomaly map
                Bv, Hv, Wv, Cv = v_early.shape
                v_fw_pred = F.normalize(students['vit_fw'](v_early.reshape(Bv, -1, Cv)).reshape(v_early.shape).float(), p=2, dim=-1)
                v_bw_pred = F.normalize(students['vit_bw'](v_late.reshape(Bv, -1, Cv)).reshape(v_late.shape).float(), p=2, dim=-1)
                
                v_map = (1 - F.cosine_similarity(v_fw_pred, F.normalize(v_late.float(), p=2, dim=-1), dim=-1)) * \
                        (1 - F.cosine_similarity(v_bw_pred, F.normalize(v_early.float(), p=2, dim=-1), dim=-1))
                v_map = v_map.unsqueeze(1)

                # LLM Anomaly map
                Bl, Hl, Wl, Cl = l_early.shape
                l_fw_pred = F.normalize(students['llm_fw'](l_early.reshape(Bl, -1, Cl)).reshape(l_early.shape).float(), p=2, dim=-1)
                l_bw_pred = F.normalize(students['llm_bw'](l_late.reshape(Bl, -1, Cl)).reshape(l_late.shape).float(), p=2, dim=-1)
                
                l_map = (1 - F.cosine_similarity(l_fw_pred, F.normalize(l_late.float(), p=2, dim=-1), dim=-1)) * \
                        (1 - F.cosine_similarity(l_bw_pred, F.normalize(l_early.float(), p=2, dim=-1), dim=-1))
                l_map = l_map.unsqueeze(1)
                ...
                # Reshape Spaziale per il PE [B, N, D]
                fw_pred = F.normalize(students['fw'](e.reshape(B, -1, C)).reshape(e.shape).float(), p=2, dim=-1)
                bw_pred = F.normalize(students['bw'](l.reshape(B, -1, C)).reshape(l.shape).float(), p=2, dim=-1)
                
                fw_err = 1 - F.cosine_similarity(fw_pred, F.normalize(l.float(), p=2, dim=-1), dim=-1)
                bw_err = 1 - F.cosine_similarity(bw_pred, F.normalize(e.float(), p=2, dim=-1), dim=-1)
                
                anomaly_map = (fw_err * bw_err).unsqueeze(1)
