...
def train(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        # Teacher
        teacher = HDViTFeatureExtractor(layers=[1, 5]).to(device, dtype=dtype).eval()
        ...
        teacher = HDLLMFeatureExtractor(conversation_template=PROMPTS_CONFIG[f'distilled_{args.class_name}'],
    ...
    for epoch in trange(args.epochs_no, desc=f'Training students for {args.class_name}...'):
        ...
        for batch_imgs in tqdm(train_loader, desc=f'    Epoch {epoch+1} [Train]'):
            ...
            earlier_input = earlier_patch.view(-1, teacher.embed_dim)
            later_input = later_patch.view(-1, teacher.embed_dim)
