...
def train(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        # Teacher
        teacher = HDLLMFeatureExtractor(conversation_template=PROMPTS_CONFIG[f'assistant_generic'],
                                        layers=[0, 1]).to(device).eval()
    ...
    with torch.no_grad():
        feat_early, _ = teacher(sample_batch) 
    
    batch_size, hd_h, hd_w, embed_dim = feat_early.shape
    num_patches = hd_h * hd_w
    print(f"Dynamic Grid Detected: {hd_h}x{hd_w} = {num_patches} total patches.")
    
    # Students
    students = {}
    students['backward_net'] = FeatureProjectionMLP_FullyTrainableSumHD(
        in_features=embed_dim, out_features=embed_dim, 
        num_patches=num_patches, act_layer=act_layer, reduction_factor=0.4
    ).to(device, dtype=dtype)

    students['forward_net'] = FeatureProjectionMLP_FullyTrainableSumHD(
        in_features=embed_dim, out_features=embed_dim, 
        num_patches=num_patches, act_layer=act_layer, reduction_factor=0.4
    ).to(device, dtype=dtype)
    ...
    for epoch in trange(args.epochs_no, desc=f'Training students for {args.class_name}...'):
        ...
        for batch_imgs in tqdm(train_loader, desc=f'    Epoch {epoch+1} [Train]'):
            ...
            # Treat every single patch as an independent data point
            b, h, w, d = earlier_patch.shape
            earlier_input = earlier_patch.reshape(b, h*w, d)
            later_input = later_patch.reshape(b, h*w, d)
            ...
            # Losses
            loss_later = 1 - cos_sim(predicted_later_patch.float(), later_input.float()).mean()
            loss_earlier = 1 - cos_sim(predicted_earlier_patch.float(), earlier_input.float()).mean()
    ...
    torch.save(students['forward_net'].state_dict(), os.path.join(save_dir, 'forward_net_' + model_name + '.pth'))
    torch.save(students['backward_net'].state_dict(), os.path.join(save_dir, 'backward_net_' + model_name + '.pth'))
    wandb.finish()
