...
def train(args):
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        pe_dim = teacher.embed_dim
        
        # Students
        students['backward_net'] = FeatureProjectionMLP_TrainableScaleSum(
            in_features=teacher.embed_dim, out_features=teacher.embed_dim, pe_dim=pe_dim, reduction_factor=0.4
        ).to(device, dtype=dtype)
        students['forward_net'] = FeatureProjectionMLP_TrainableScaleSum(
            in_features=teacher.embed_dim, out_features=teacher.embed_dim, pe_dim=pe_dim, reduction_factor=0.4
        ).to(device, dtype=dtype)
    ...
    # Optimizer
    alpha_params = [s.pe_alpha for s in students.values()]
    weight_params = [p for s in students.values() for n, p in s.named_parameters() if "pe_alpha" not in n]

    optimizer = torch.optim.Adam([
        {'params': weight_params, 'lr': 1e-4},
        {'params': alpha_params, 'lr': 1e-2} 
    ])
    ...
    for epoch in trange(args.epochs_no, desc=f'Training students for {args.class_name}...'):
        ...
        for pil_img, tensor_img in tqdm(train_loader, desc=f'    Epoch {epoch+1} [Train]'):
            ...
            # Nets prediction
            predicted_later_patch = students['forward_net'](earlier_patch, pe_matrix)
            predicted_earlier_patch = students['backward_net'](later_patch, pe_matrix)
        ...
        wandb.log({
            "params/pe_alpha_forward": students['forward_net'].pe_alpha.item(),
            "params/pe_alpha_backward": students['backward_net'].pe_alpha.item()
            })
