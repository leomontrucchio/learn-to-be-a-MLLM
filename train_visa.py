...
def train(args):
    ...
    # Conversation template (just if Both_LLM)
    conv_template = None
    if args.students_blocks == 'Both_LLM':
    ...
    if args.students_blocks == 'Both_ViT':
        ...
        teachers['llm_fe'] = LLMFeatureExtractor(
            conversation_template=conv_template,
            layer1_idx=0, layer2_idx=1
        ).to(device).eval()
        ...
        # Teacher
        teachers['fe'] = LLMFeatureExtractor(
            conversation_template=conv_template,
            layer1_idx=0, layer2_idx=1
        ).to(device).eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--students_blocks', type=str, default='Both_LLM', choices=['Both_ViT', 'ViT-LLM', 'Both_LLM'],
